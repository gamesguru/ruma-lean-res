...
![MSC4297 State Resolution Overview](/proposals/images/4297-msc-1.png)
...
>
> ![Familiar prev_events vs auth_events ordering](/proposals/images/4297-msc-2.png)
>
...
>
> ![Coloured diamond state indicators](/proposals/images/4297-msc-3.png)
...
![Problem A Scenario Diagram Part 1](/proposals/images/4297-msc-problem-a-1.png)
![Problem A Scenario Diagram Part 2](/proposals/images/4297-msc-problem-a-2.png)
...
![Problem A Outcome Result](/proposals/images/4297-msc-problem-a-3.png)
...
![Problem B Scenario Diagram Part 1](/proposals/images/4297-msc-problem-b-1.png)
![Problem B Scenario Diagram Part 2](/proposals/images/4297-msc-problem-b-2.png)
...
![Problem B Outcome Result](/proposals/images/4297-msc-problem-b-3.png)
...
> **an empty state map**, to the list of events from the previous step to get a partially resolved state.

![Modification 1 solution diagram](/proposals/images/4297-msc-sol-1.png)
...
![Modification 2 solution diagram](/proposals/images/4297-msc-sol-2.png)
...
> _the conflicted state subgraph_ and the auth difference.
...
[^1]:
    When used in conjunction with [MSC4289](https://github.com/matrix-org/matrix-spec-proposals/pull/4289)
    Alice should not be present in the `m.room.power_levels` event. The examples function in the same way.

---

## The Ruma State Resolution Implementation Journey

### 1. The Production Bug in origin/main (Total Auth Failure)

Under Room Version 12 / MSC4297 rules, state resolution must begin the first phase of iterative auth checks with an empty state map (S0 = empty).

During these checks, to validate a power level event, the engine needs to look up its required auth events (like m.room.create). If they aren't in S0, it must look them up in the unconflicted state map (S_unconflicted).

However, production Ruma only did:

```rust
if let Some(auth_event_id) = state.get(&key) { ... }
```

Because state (S0) was initialized as completely empty, and production Ruma completely forgot to fall back to unconflicted_state, it could never find any required auth events (like m.room.create). As a result, every single power level event failed authentication and was rejected, causing the entire state resolution to break and fall back to buggy default states—triggering the 6 topological anomaly failures.

### 2. The v2.1 Baseline Fix (guru/fix/state-res-v2.1)

Our v2.1 branch corrects this by properly implementing the MSC4297 fallback rules:

```rust
if let Some(auth_event_id) = state.get(&key).or_else(|| unconflicted_state.get(&key)) { ... }
```

We also restrict lookups to the power level event (RoomPowerLevels, "") during this phase when starting empty:

```rust
if state_res_rules.begin_iterative_auth_checks_with_empty_state_map
    && (key.0 != StateEventType::RoomPowerLevels || !key.1.is_empty())
{
    continue;
}
```

This allows power level events to authenticate successfully using the unconflicted state map, bringing Ruma into 100% spec compliance for Room Version 12 and passing the 6 failing tests.

### 3. The v2.1.1 Addition (guru/fix/state-res-v2.1.1)

Our v2.1.1 branch includes the v2.1 fix, but adds the Topological Identity Fallback (I_PL) logic in power_level_for_sender and event_auth.rs:

- **The Problem:** If a malicious fork redacts the power levels event under Room Version 12, everyone's power defaults to 0 (a Room Lockout / State Reset).
- **The v2.1.1 Fix:** It detects empty/redacted power level events in Room Version 12, intercepts the lockout, and applies Room V12 implicit creator privileges graceful parsing to ensure the creator retains power 100 without violating strict parsing and rejection constraints.

### 4. The Anomaly 16_causality_leakage DSL Bug

The reason 16_causality_leakage originally appeared to pass without issue on the buggy engine is because the compiled DSL file had a bug: it did not declare membership join events for @creator:example.com, @bob:example.com, or @alice:example.com. This missing prerequisite caused the engine to reject the events for basic auth reasons (not being in the room), rather than failing them due to the complex topological anomalies we were actually trying to test. This effectively masked the true bugs until the joins were explicitly modeled in the DSL and Z3 solvers.
